...
class Server_Controller(db.Model):
	...
	def run(self):
		...
		for title in self.l_movie_titles[self.CURRENT_MOVIE:]:
			if (movie_counter == self.MAX_VIDEOS):
				break
			videoIDs_JSON = requests.post('http://localhost:8000/api/videos/'+ title, headers={'Authorization': 'Bearer '+self.access_token})
	...
	def get_videos(self, title):
		self.CURRENT_API = 1
		videoIDs_JSON = requests.post('http://localhost:8000/api/youtube_list/'+ title, headers={'Authorization': 'Bearer '+self.access_token})
	...
	def get_comment_threads(self, video_id):
		self.CURRENT_API = 2
		response = requests.get('http://localhost:8000/api/comment_threads/'+video_id, headers={'Authorization': 'Bearer '+self.access_token})
		if (response.status_code != requests.codes.ok):
			pass

	def get_video_captions(self, video_id):
		self.CURRENT_API = 1
		response = requests.get('http://localhost:8000/api/video_caption/'+video_id, headers={'Authorization': 'Bearer '+self.access_token})
		if (response.status_code != requests.codes.ok):
			pass
